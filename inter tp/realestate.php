...
if (!empty($location)) {
    $query .= " AND make LIKE '%$location%'"; // assuming 'make' field captures location for real estate
}
if (!empty($type)) {
    $query .= " AND model LIKE '%$type%'"; // assuming 'model' field captures type of real estate
}
if (!empty($min_price)) {
...
<body>
    ...
    <div class="form-row">
        <div class="form-group">
            <label for="make">Make/location:</label>
    ...
    <h2>Available Real Estate</h2>
    <?php while ($row = $result->fetch_assoc()): ?>
        <div class="product-listing">
            <strong>Location:</strong> <?= htmlspecialchars($row['make']) ?><br>
            <strong>Type:</strong> <?= htmlspecialchars($row['model']) ?><br>
